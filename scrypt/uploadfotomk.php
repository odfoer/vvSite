...
<html lang="uk">
    ...
    <body>
        ...
        if (isset($description)) {
                ...
                echo ("del");
                $path_directory = '../images/makeup/';
                $result = $mysqli->query("SELECT * FROM makeupfoto WHERE id='$id'");
                $myrow = $result->fetch_assoc();
                if ($id == $myrow['id']){ 
                echo "yes-";
                echo $myrow['mini'];
                echo $myrow['big'];
                $mysqli->query("DELETE FROM makeupfoto WHERE id='$id'");
        ...
        unlink ($delfull);
                ...
                echo $myrow['mini'];
                echo $myrow['big'];
                $mysqli->query("UPDATE makeupfoto SET mini='$date', big='$date2' WHERE id='$id'");
                $old = $path_directory.$myrow['mini'].'.jpg';
                $old2 = $path_directory.$myrow['big'].'.jpg';
                echo $old.$old2;
                unlink ($old);  
                unlink ($old2);         
                        ...
                        if ($id > $myrow['id']){ 
                $mysqli->query("INSERT INTO makeupfoto(id,mini,big) VALUES ('$id','$date','$date2')");
                }
                }
				}} 
                ?>
