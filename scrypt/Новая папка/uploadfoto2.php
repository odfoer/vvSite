...
<html lang="uk">
    ...
    <body>
        ...
        if (isset($description)) {
                ...
                echo ("del");
                $path_directory = '../img/night/';
                $result = mysql_query("SELECT * FROM nightfoto WHERE id='$id'",$db);
                $myrow = mysql_fetch_assoc($result);
                if ($id == $myrow['id']){ 
                echo "yes-";
                echo $myrow['fotobig'];
                echo $myrow['foto'];
                mysql_query("DELETE FROM nightfoto WHERE id='$id'",$db);
        ...
        unlink ($delfull);
                ...
                echo $myrow['fotobig'];
                echo $myrow['foto'];
                mysql_query("UPDATE nightfoto SET fotobig='$date', foto='$date2' WHERE id='$id'",$db);
                $old = $path_directory.$myrow['fotobig'].'.jpg';
                $old2 = $path_directory.$myrow['foto'].'.jpg';
                echo $old.$old2;
                unlink ($old);  
                unlink ($old2);         
                        ...
                        if ($id > $myrow['id']){ 
                mysql_query("INSERT INTO nightfoto(id,fotobig,foto) VALUES ('$id','$date','$date2')",$db);
                }
                }}} 
                ?>
    </body>
