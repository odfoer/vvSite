...
<html lang="uk">
    ...
    <body>
        ...
        if (isset($description)) {
                ...
                $result = $mysqli->query("SELECT * FROM newsgallery WHERE id='$id'");
                ...
                echo $myrow['mini'];
                echo $myrow['big'];
                $mysqli->query("DELETE FROM newsgallery WHERE id='$id'");
		...
		?>
				<script language="JavaScript" type="text/javascript">
					<!-- 
					location="../admin/anews&events_.php?theme=<?php echo $theme; ?>" 
					//--> 
				</script>
		<?php 
        exit;
        ...
        unlink ($delfull);
                ...
                echo $myrow['mini'];
                echo $myrow['big'];				
                $mysqli->query("UPDATE newsgallery SET mini='$date', big='$date2' WHERE id='$id'");
                $old = $path_directory.$myrow['mini'].'.jpg';
                $old2 = $path_directory.$myrow['big'].'.jpg';
                echo $old.$old2;
                unlink ($old);  
                unlink ($old2);         
						...
						$theme = $mysqli->real_escape_string($_POST['theme']);
                                echo ("insert".$id."||||".$theme);
                $mysqli->query("INSERT INTO newsgallery(id,theme,mini,big) VALUES ('$id','$theme','$date','$date2')");
                }
                }
				}} 
                ?>
